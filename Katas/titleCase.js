  ...
  return title
    ...
    .map((word, index) =>
      index === 0
        ...
        : minorWords.toUpperCase().split(" ").includes(word.toUpperCase())
        ? word.toLowerCase()
        : word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()
    )
    .join(" ");
...
//   (that is, only the first letter of the word is in upper case) or (b) considered to be an exception and put entirely into lower case
