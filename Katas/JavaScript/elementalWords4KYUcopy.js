...
function elementalForms(word) {
  ...
  while (true) {
    ...
    // upper case first char of all symbols in matches
    ...
    matches.forEach((match, index) => {
      ...
      // delete tempElements[matches[index].symbol];
      // mutating affects ELEMENTS, so reduce works to avoid mutating the object
    ...
    // break once we're out of matches
    if (tempMatches.length === 0) break;
  }

  // ok now remove the matches from the regex string, reformat the regex and scan again for what's missing

  ////////////////////////////////
  // sort the matched word into the desired output
  console.log(matches);
  //////////////////////////////////////// fuck yeah, got all the seperate matches now.
  ///////////////////// now we need a function to find the total combinations to create the full word
  if (tempElements === ELEMENTS) return [];
  return;
}

// console.log(elementalForms("beach"));
console.log(elementalForms("snack"));
// console.log(elementalForms("rob"));
